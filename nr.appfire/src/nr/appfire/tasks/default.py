...
class Task(api.Task[T]):
  ...
  @property
  def runnable(self) -> Runnable: return self._runnable
  ...
  def _mainloop(self) -> None:
    while True:
      ...
      if task is None:
        break
      if task.cancelled():
        task._update(TaskStatus.IGNORED)
...
class DefaultExecutor(api.Executor):
  ...
  def execute(
    ...
    assert isinstance(runnable, Runnable), f'expected instance of Runnable, got {type(runnable).__name__} instead'
    if self._shutdown:
